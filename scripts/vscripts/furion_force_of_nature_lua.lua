...
function furion_force_of_nature_lua:OnSpellStart()
    ...
    while nTreantsToSpawn > 0 do
        ...
        local waaaall = CreateUnitByName( "npc_treetag_building_wall_1", vTargetPosition, true, self:GetCaster(), self:GetCaster():GetOwner(), self:GetCaster():GetTeamNumber() )
        if waaaall ~= nil then
            waaaall:SetControllableByPlayer( self:GetCaster():GetPlayerID(), false )
            waaaall:SetOwner( self:GetCaster() )
        end
