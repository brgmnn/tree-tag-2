...
LinkLuaModifier("modifier_furion_force_of_nature_lua",
...
function furion_force_of_nature_lua:OnSpellStart()
    ...
    local trees = GridNav:GetAllTreesAroundPoint(vTargetPosition,
    ...
    ParticleManager:SetParticleControl(nFXIndex, 2,
            Vector(self.area_of_effect, 0, 0))
    ...
    while nTreantsToSpawn > 0 do
        local hTreant = CreateUnitByName("npc_dota_furion_treant",
                vTargetPosition, true, self:GetCaster(),
                self:GetCaster():GetOwner(), self:GetCaster():GetTeamNumber())
        if hTreant ~= nil then
            hTreant:SetControllableByPlayer(self:GetCaster():GetPlayerID(),
                    false)
            ...
            hTreant:AddNewModifier(self:GetCaster(), self,
                    "modifier_furion_force_of_nature_lua", kv)
        end

        local wl = CreateUnitByName("npc_treetag_building_wall_1",
                vTargetPosition, true, self:GetCaster(),
                self:GetCaster():GetOwner(), self:GetCaster():GetTeamNumber())

        if wl ~= nil then
            wl:SetControllableByPlayer( self:GetCaster():GetPlayerID(), false )
            wl:SetOwner( self:GetCaster() )
        end

        nTreantsToSpawn = nTreantsToSpawn - 1
    end

    EmitSoundOnLocationWithCaster(vTargetPosition, "Hero_Furion.ForceOfNature",
            self:GetCaster())
end
